...
The number of possible combinations to put n items into one bag is .
...
Remember that a given item can only be in bag1, bag2, or neither bag -- it is not possible for an item
...
x = yieldAllCombos(['Book','Phone']) # 3**2 = 9 possible combinations
for n in range(0, 9):
    print(next(x))
