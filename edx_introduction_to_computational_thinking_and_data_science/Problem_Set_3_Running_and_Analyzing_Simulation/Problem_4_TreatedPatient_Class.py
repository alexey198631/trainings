"""
We also need a representation for a patient that accounts for the use of drug treatments and manages a collection
of ResistantVirus instances. For this, we introduce the TreatedPatient class, which is a subclass of Patient.
...
What happens when a drug is introduced? The drugs we consider do not directly kill virus particles lacking
resistance to the drug, but prevent those virus particles from reproducing (much like actual drugs used to treat HIV).
Virus particles with resistance to the drug continue to reproduce normally. Implement the TreatedPatient class.
"""
from Problem_1_Implementing_a_Simple_Simulation_No_Drug_Treatment import *
...
class ResistantVirus(SimpleVirus):
    ...
    def __init__(self, maxBirthProb, clearProb, resistances, mutProb):
        ...
        self.resistances = resistances  # 'guttagonol':False, 'srinol':False
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if all([self.isResistantTo(i) for i in self.activeDrugs]) == True:
            repr_probability = random.random()
            if repr_probability <= self.maxBirthProb * (1 - popDensity):
                new_resist = self.resistances.copy()
