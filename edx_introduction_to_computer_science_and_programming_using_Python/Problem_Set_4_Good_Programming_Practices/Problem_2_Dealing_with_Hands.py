...
letters from this set are used up.
...
l, m. Your task is to implement the function updateHand, which takes in two inputs - a hand and a word (string).
updateHand uses letters from the hand to spell the word, and then returns a copy of the hand, containing only the
letters remaining.
...
def updateHand(hand, word):
    ...
    for letter in word:
        for k in updateHand:
            if letter == k:
                updateHand[k] = updateHand.get(k, 0) - 1
    return updateHand